...
**One-paste path:** paste `seed/setup_full_catalog.sql` into SQL Editor and Run —
it is generated by `node seed/generate_setup.mjs` from the four files below, in
the order they must run, and it is fully idempotent.

Same place, one file at a time (use this when you want a result grid per step):
...
> It is written as a set-based bulk load (~385 KB, ~25 statements) rather than
> one statement per lesson, which is what keeps a paste that size responsive in
> the SQL Editor. See [`seed/README_RUNBOOK.md`](seed/README_RUNBOOK.md).
