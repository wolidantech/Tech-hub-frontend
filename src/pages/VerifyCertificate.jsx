    ...
    <div className="min-h-screen bg-gradient-to-br from-[#020a1f] via-[#061236] to-[#020a1f] py-16">
      <div className="mx-auto max-w-[640px] px-4 sm:px-6 lg:px-8">
        <div className="text-center space-y-4 mb-10">
          ...
          <p className="text-white/60">Enter a Certificate ID to verify its authenticity. Employers and institutions can use this tool.</p>
        </div>
        ...
        <div className="glass-strong rounded-[24px] p-8 space-y-6">
          <form onSubmit={handleVerify} className="space-y-4">
            <div className="relative">
              ...
              <input value={id} onChange={e => setId(e.target.value)} placeholder="Enter Certificate ID (e.g. WDTH-2026-ABC123)" className="w-full h-[56px] rounded-full glass pl-12 pr-4 font-mono text-sm placeholder:font-sans placeholder:text-white/40 focus:outline-none focus:border-cyan-400/50 focus:bg-white/[0.08] transition" required />
            </div>
            ...
            <div className="pt-6 border-t border-white/10">
              {result ? (
                <div className="space-y-6">
                  <div className="flex items-center gap-3 text-green-300 font-bold"><CheckCircle2 className="h-6 w-6" /> Certificate Valid ✅</div>
                  <div className="rounded-2xl bg-green-500/10 border border-green-500/20 p-6 space-y-4">
                    <div className="flex justify-between text-sm"><span className="text-white/50">Student Name</span><span className="font-bold flex items-center gap-1"><User className="h-4 w-4" /> {result.studentName || 'Student'}</span></div>
                    ...
                    <div className="flex justify-between text-sm"><span className="text-white/50">Issuer</span><span className="font-bold">WOLI DAN TECH HUB</span></div>
                  </div>
                ...
                <div className="text-center space-y-4">
                  ...
                  <div><div className="font-bold text-lg">Certificate Not Found ❌</div><div className="text-sm text-white/50 mt-1">No certificate matches ID: <span className="font-mono text-white">{id}</span></div></div>
        ...
        <div className="mt-8 glass rounded-2xl p-6 text-sm text-white/50 leading-relaxed">
          ...
          <ul className="list-disc pl-5 space-y-1">
            <li>Every certificate issued by WOLI DAN TECH HUB has a unique ID</li>
            <li>Enter the ID above to confirm authenticity</li>
